...
pub fn get_data_validations_nif<'a>(
            ...
            if let Some(data_validations) = sheet.get_data_validations() {
                for validation in data_validations.get_data_validation_list() {
                    ...
                    map = map
                        .map_put(
                            atoms::rule_type(),
                            rustler::types::atom::Atom::from_str(env, &validation_type).unwrap(),
                        )
                        .ok()
                        .unwrap();
                    ...
                    if *validation.get_type() != DataValidationValues::List
                        && *validation.get_type() != DataValidationValues::Custom
                    {
                        let op_str = operator_to_string(validation.get_operator());
                        map = map
                            .map_put(
                                atoms::operator(),
                                rustler::types::atom::Atom::from_str(env, &op_str).unwrap(),
                            )
                            .ok()
                            .unwrap();
                    }
                    ...
                    if !validation.get_formula1().is_empty() {
                        map = map
                            .map_put(atoms::formula1(), validation.get_formula1().to_string())
                            .ok()
                            .unwrap();
                    }
                    ...
                    if !validation.get_formula2().is_empty() {
                        map = map
                            .map_put(atoms::formula2(), validation.get_formula2().to_string())
                            .ok()
                            .unwrap();
                    }
                    ...
                    if *validation.get_show_error_message() {
                        map = map
                            .map_put(
                                atoms::show_error_message(),
                                validation.get_show_error_message(),
                            )
                            .ok()
                            .unwrap();
                        if !validation.get_error_title().is_empty() {
                            map = map
                                .map_put(
                                    atoms::error_title(),
                                    validation.get_error_title().to_string(),
                                )
                                .ok()
                                .unwrap();
                        }
                        if !validation.get_error_message().is_empty() {
                            map = map
                                .map_put(
                                    atoms::error_message(),
                                    validation.get_error_message().to_string(),
                                )
                                .ok()
                                .unwrap();
                        }
                    ...
                    if *validation.get_show_input_message() {
                        map = map
                            .map_put(
                                atoms::show_input_message(),
                                validation.get_show_input_message(),
                            )
                            .ok()
                            .unwrap();
                        if !validation.get_prompt_title().is_empty() {
                            map = map
                                .map_put(
                                    atoms::prompt_title(),
                                    validation.get_prompt_title().to_string(),
                                )
                                .ok()
                                .unwrap();
                        }
                        if !validation.get_prompt().is_empty() {
                            map = map
                                .map_put(
                                    atoms::prompt_message(),
                                    validation.get_prompt().to_string(),
                                )
                                .ok()
                                .unwrap();
                        }
                    ...
                    map = map
                        .map_put(atoms::allow_blank(), validation.get_allow_blank())
                        .ok()
                        .unwrap();
    ...
    match result {
        ...
        Err(_) => Err((
            atoms::error(),
            "Error occurred in get_data_validations".to_string(),
        )),
    }
}
...
pub fn get_list_validations_nif<'a>(
    ...
    match result {
        ...
        Err(_) => Err((
            atoms::error(),
            "Error occurred in get_list_validations".to_string(),
        )),
    }
}
...
pub fn get_number_validations_nif<'a>(
        ...
        || -> Result<Vec<rustler::Term<'a>>, String> {
            ...
            if let Some(data_validations) = sheet.get_data_validations() {
                for validation in data_validations.get_data_validation_list() {
                    ...
                    if *validation.get_type() != DataValidationValues::Decimal
                        && *validation.get_type() != DataValidationValues::Whole
                    {
                        continue;
                    }
                    ...
                    map = map
                        ...
                        .ok()
                        .unwrap();
                    let type_name = if *validation.get_type() == DataValidationValues::Decimal {
                        "decimal"
                    } else {
                        "whole"
                    };
                    map = map
                        .map_put(
                            atoms::rule_type(),
                            rustler::types::atom::Atom::from_str(env, type_name).unwrap(),
                        )
                        .ok()
                        .unwrap();
                    ...
                    // Formula values
                    map = map
                        .map_put(atoms::value1(), validation.get_formula1().to_string())
                        .ok()
                        .unwrap();
                    if !validation.get_formula2().is_empty() {
                        map = map
                            .map_put(atoms::value2(), validation.get_formula2().to_string())
                            .ok()
                            .unwrap();
                    }
    ...
    match result {
        ...
        Err(_) => Err((
            atoms::error(),
            "Error occurred in get_number_validations".to_string(),
        )),
    }
}
...
pub fn get_date_validations_nif<'a>(
        ...
        || -> Result<Vec<rustler::Term<'a>>, String> {
            ...
            if let Some(data_validations) = sheet.get_data_validations() {
                for validation in data_validations.get_data_validation_list() {
                    ...
                    // Date values
                    map = map
                        .map_put(atoms::date1(), validation.get_formula1().to_string())
                        .ok()
                        .unwrap();
                    if !validation.get_formula2().is_empty() {
                        map = map
                            .map_put(atoms::date2(), validation.get_formula2().to_string())
                            .ok()
                            .unwrap();
                    }
    ...
    match result {
        ...
        Err(_) => Err((
            atoms::error(),
            "Error occurred in get_date_validations".to_string(),
        )),
    }
}
...
pub fn get_text_length_validations_nif<'a>(
            ...
            if let Some(data_validations) = sheet.get_data_validations() {
                for validation in data_validations.get_data_validation_list() {
                    ...
                    map = map
                        ...
                        .unwrap();
                    map = map
                        .map_put(atoms::rule_type(), atoms::text_length())
                        .ok()
                        .unwrap();
    ...
    match result {
        ...
        Err(_) => Err((
            atoms::error(),
            "Error occurred in get_text_length_validations".to_string(),
        )),
    }
}
...
pub fn get_custom_validations_nif<'a>(
            ...
            if let Some(data_validations) = sheet.get_data_validations() {
                for validation in data_validations.get_data_validation_list() {
                    ...
                    map = map
                        ...
                        .unwrap();
                    map = map
                        .map_put(atoms::rule_type(), atoms::custom())
                        .ok()
                        .unwrap();

                    // Formula
                    map = map
                        .map_put(atoms::formula(), validation.get_formula1().to_string())
                        .ok()
                        .unwrap();
    ...
    match result {
        ...
        Err(_) => Err((
            atoms::error(),
            "Error occurred in get_custom_validations".to_string(),
        )),
    }
}
...
pub fn has_data_validations_nif(
    ...
    match result {
        ...
        Err(_) => Err((
            atoms::error(),
            "Error occurred in has_data_validations".to_string(),
        )),
    }
}
...
pub fn count_data_validations_nif(
    ...
    match result {
        ...
        Err(_) => Err((
            atoms::error(),
            "Error occurred in count_data_validations".to_string(),
        )),
    }
}
