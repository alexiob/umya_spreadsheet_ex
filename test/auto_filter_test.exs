defmodule UmyaSpreadsheetTest.AutoFilterTest do
  ...
  test "set_auto_filter adds an auto filter to the specified range", %{spreadsheet: spreadsheet} do
    ...
    assert UmyaSpreadsheet.has_auto_filter(spreadsheet, "Sheet1")

    # Check that the filter range is correct
    assert UmyaSpreadsheet.get_auto_filter_range(spreadsheet, "Sheet1") == range
  end
  ...
  test "remove_auto_filter removes an auto filter from the worksheet", %{spreadsheet: spreadsheet} do
    ...
    refute UmyaSpreadsheet.has_auto_filter(spreadsheet, "Sheet1")
    assert UmyaSpreadsheet.get_auto_filter_range(spreadsheet, "Sheet1") == nil
  end
  ...
  test "auto filter persists when saving and loading the workbook", %{spreadsheet: spreadsheet} do
    ...
    temp_path = "auto_filter_test.xlsx"
    ...
    assert UmyaSpreadsheet.has_auto_filter(loaded_spreadsheet, "Sheet1")
    assert UmyaSpreadsheet.get_auto_filter_range(loaded_spreadsheet, "Sheet1") == "A1:C4"
